...
public class SessionBuilder {
	...
	public SessionBuilder(long defaultExpires, int limit, int rememberDay,
		...
		Subject.init(rememberDay, new Credentials(authenticateService,
				defaultExpires), authenticateService.getPasswordService());
	}
	...
	public Session out(Session oldSession, HttpResponse response) {
		// TODO cqiyi：如果request的header中有apikey，则不通过cookies生成，直接退出
		...
		if (session != oldSession) {
			...
			// 现在的session
			sessions.update(oldUsername, oldSessionKey, username, sessionKey,
					session);
		} else {
	...
	private Session buildSession(HttpRequest request) {
		...
		if (cookie != null) {
			...
			String sig = request.getCookieValue(sessionCookieDescriptor
					.getCookieSignatureName());
			...
			if (sig != null && signer.verify(cookie, sig)) {
				...
				String principalName = cookieValues
						.get(Principal.PRINCIPAL_DEF_KEY);
				if (principalName != null && !"".equals(principalName.trim())) {
					// 判断 是否使用了 remeberme 或失效
					Sessions.SessionDatas sessionDatas = sessions
							.get(principalName);
					if (sessionDatas != null) {
						String sessionKey = cookieValues
								.get(Session.SESSION_DEF_KEY);
						Sessions.SessionData sessionData = sessionDatas
								.getSessionData(sessionKey);
						if (sessionData != null) {
							logger.debug(
									"Found session success. username was: %s.",
									principalName);
							return sessionData.getSession();
						}
					} else {
						logger.warn(
								"Invalid user session. session was: %s. Ignoring session cookie.",
								cookie);
					}
				} else {
					logger.warn(
							"Invalid username. session was: %s. Ignoring session cookie.",
							cookie);
				}
			} else {
				logger.warn(
						"Invalid session signature. session was: %s. Ignoring session cookie.",
						cookie);
			}
		}
	...
	private void updateCookie(HttpResponse resp, Session session) {
		...
		if (cookiesMap.isEmpty()) {
			...
			for (Map.Entry<String, String> cookie : cookiesMap.entrySet()) {
				resp.addCookie(cookie.getKey(), cookie.getValue(), session
						.getExpires() > 0 ? (int) (session.getExpires() / 1000)
						: (int) session.getExpires());
			}
		}
	}
	...
	private Map<String, String> getCookiesMap(Session session) {
		...
		Map<String, String> sessionMap = Maper.of(Session.SESSION_DEF_KEY,
				session.getSessionKey());
		Principal principal = session.getPrincipal();
		if (principal != null) {
			sessionMap
					.put(Principal.PRINCIPAL_DEF_KEY, principal.getUsername());
		} else {
			sessionMap.put(Principal.PRINCIPAL_DEF_KEY,
					getAnonymousName(session));
		}
		String sessionJson = Jsoner.toJSON(sessionMap);
		String sessionJsonSign = signer.sign(sessionJson);
		logger.info("Save cookie " + sessionCookieDescriptor.getCookieName()
				+ ": " + sessionJson + ", "
				+ sessionCookieDescriptor.getCookieSignatureName() + ": "
				+ sessionJsonSign);
